...
- [ ] 1.1 Add a `Workspace` type in `server/src/` owning what `index.ts` holds at module level: agent runtime, sandbox settings, browser root, writable root, git state, file watcher, sandboxed toolset, session manager, work plan
- [ ] 1.2 Give it a lifecycle: `open()` builds those resources for a root directory, `stop()` releases them (watcher closed, session disposed), `isBusy()` reports whether a turn is running
