...
- [ ] 4.3 Exercise the running app end to end: ask the agent to create a nested plan, verify the first tool call, persisted sidecar, visible hierarchy, generated IDs, later task update, and resume/read behavior. Server, sidecar, update, and reconnect boundaries are verified; the running-app DOM assertion remains blocked by the in-app browser's localhost security policy.
...
- [ ] 4.5 Treat tasks 1.3, 2.4, and 4.4 as explicit completion gates, then run strict OpenSpec validation and reconcile the Work Plan and scenario matrix before marking the change complete.
