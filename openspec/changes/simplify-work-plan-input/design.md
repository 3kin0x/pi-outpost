...
The plan ID and every omitted task ID are server-owned, JSON-safe strings generated before validation. A task ID the agent supplies is adopted instead, and a collection whose supplied IDs are not unique is rejected atomically. Recursive normalization tracks parent IDs and rejects invalid nesting or limit violations before persistence. IDs are unique within the candidate plan, stable after creation, and remain unchanged by renames and moves. The compatibility `add_task` path is unchanged.

Refusing caller-supplied IDs was tried first and failed against a real model. The published schema is a ten-branch root `anyOf`, delivered with `strict: false`, so no constrained decoding holds the model to one branch: `mistral-medium-latest` merged the neighbouring `add_task`/`replace` task shape into `create` and sent an `id` on every task. The rejection is then unusable as repair guidance, because a failed union reports "must not have additional properties" once per branch without naming the property; in the running app the model deleted its `description` fields — which were valid — and its whole second level, and still failed. Accepting the ID it chose costs nothing (identity stays unique either way), removes the repair round entirely, and lets the model address its own tasks in a later `update_task` without an intervening `get`.
